...
do_process_hello_retry_request(SSL_HANDSHAKE *hs) {
	...
	if (have_key_share) {
		 ...
		 * the key share that was provided in the initial
		 * ClientHello.
		 */
...
static enum ssl_hs_wait_t
do_send_second_client_hello(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
	/*
	 * TODO(svaldez): Ensure that we set can_early_write to false
	 * since 0-RTT is rejected if we receive a HelloRetryRequest.
	 */
...
static enum ssl_hs_wait_t
do_process_server_hello(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
	...
	const SSL_EXTENSION_TYPE ext_types[] = {
		{TLSEXT_TYPE_key_share, &have_key_share, &key_share},
		{TLSEXT_TYPE_pre_shared_key, &have_pre_shared_key,
		 &pre_shared_key},
	};
	...
	if (have_pre_shared_key) {
		...
		hs->new_session = SSL_SESSION_dup(ssl->session,
	...
	if (!have_key_share) {
		/*
		 * We do not support psk_ke and thus always require a
		 * key share.
		 */
	...
	alert = SSL_AD_DECODE_ERROR;
	if (!ssl_ext_key_share_parse_serverhello(hs, &dhe_secret,
...
static enum ssl_hs_wait_t
do_process_encrypted_extensions(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_process_certificate_request(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_process_server_certificate(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_process_server_finished(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_send_end_of_early_data(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_send_client_certificate(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_send_client_certificate_verify(SSL_HANDSHAKE *hs,
int is_first_run) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_complete_second_flight(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
enum ssl_hs_wait_t
tls13_client_handshake(SSL_HANDSHAKE *hs) {
	while (hs->tls13_state != state_done) {
		...
		case state_send_client_certificate_verify:
			/* first run */
			ret = do_send_client_certificate_verify(hs, 1);
			break;
		case state_complete_client_certificate_verify:
			/* complete */
			ret = do_send_client_certificate_verify(hs, 0);
			break;
...
int
tls13_process_new_session_ticket(SSL *ssl) {
	int ret = 0;
	...
	if (!CBS_get_u32(&cbs, &server_timeout) ||
	    ...
	    !CBS_stow(&ticket, &session->tlsext_tick,
	...
	/*
	 * Cap the renewable lifetime by the server advertised
	 * value. This avoids wasting bandwidth on 0-RTT when we know
	...
	if (have_early_data_info && ssl->cert->enable_early_data) {
		if (!CBS_get_u32(&early_data_info,
	...
	if (ssl->ctx->new_session_cb != NULL &&
		...
		/*
		 * |new_session_cb|'s return value signals that it
		 *  took ownership.
		 */
		session = NULL;
	}
...
void
ssl_clear_tls13_state(SSL_HANDSHAKE *hs) {
	SSL_ECDH_CTX_cleanup(&hs->ecdh_ctx);
