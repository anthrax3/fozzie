...
static int
init_key_schedule(SSL_HANDSHAKE *hs, uint16_t version,
int algorithm_prf) {
...
int
tls13_init_key_schedule(SSL_HANDSHAKE *hs) {
...
int
tls13_init_early_key_schedule(SSL_HANDSHAKE *hs) {
...
int
tls13_advance_key_schedule(SSL_HANDSHAKE *hs, const uint8_t *in,
size_t len) {
...
static int
hkdf_expand_label(uint8_t *out, const EVP_MD *digest,
...
/*
 * derive_secret derives a secret of length |len| and writes the
 * result in |out| with the given label and the current base secret
 * and most recently-saved handshake context. It returns one on
 * success and zero on error.
 */
static int
derive_secret(SSL_HANDSHAKE *hs, uint8_t *out, size_t len,
const uint8_t *label, size_t label_len) {
	...
	return hkdf_expand_label(out, SSL_TRANSCRIPT_md(&hs->transcript),
	    hs->secret, hs->hash_len, label, label_len, context_hash,
	    context_hash_len, len);
}

int
tls13_set_traffic_key(SSL *ssl, enum evp_aead_direction_t direction,
	...
	SSL_AEAD_CTX *traffic_aead = SSL_AEAD_CTX_new( direction, version,
...
int
tls13_derive_early_secrets(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
int
tls13_derive_handshake_secrets(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
int
tls13_derive_application_secrets(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
int
tls13_rotate_traffic_key(SSL *ssl, enum evp_aead_direction_t direction) {
	const EVP_MD *digest = ssl_get_handshake_digest(
	    SSL_get_session(ssl)->cipher->algorithm_prf,
...
int
tls13_derive_resumption_secret(SSL_HANDSHAKE *hs) {
...
static int
tls13_verify_data(const EVP_MD *digest, uint8_t *out,
	...
	if (!hkdf_expand_label(key, digest, secret, hash_len,
	    ...
	    HMAC(digest, key, hash_len, context, context_len, out, &len) ==
	    NULL) {
		return 0;
	}
	*out_len = len;
...
int
tls13_finished_mac(SSL_HANDSHAKE *hs, uint8_t *out, size_t *out_len,
int is_server) {
	const uint8_t *traffic_secret;
	if (is_server) {
...
int
tls13_export_keying_material(SSL *ssl, uint8_t *out, size_t out_len,
...
static int
tls13_psk_binder(uint8_t *out, const EVP_MD *digest, uint8_t *psk,
    size_t psk_len, uint8_t *context,
    size_t context_len, size_t hash_len) {
	uint8_t binder_context[EVP_MAX_MD_SIZE];
	unsigned binder_context_len;
	if (!EVP_Digest(NULL, 0, binder_context, &binder_context_len, digest,
	    NULL)) {
		return 0;
	}
...
int
tls13_write_psk_binder(SSL_HANDSHAKE *hs, uint8_t *msg, size_t len) {
	SSL *const ssl = hs->ssl;
...
int
tls13_verify_psk_binder(SSL_HANDSHAKE *hs, SSL_SESSION *session,
CBS *binders) {
	size_t hash_len = SSL_TRANSCRIPT_digest_len(&hs->transcript);

	/*
	 * Get the full ClientHello, including message header. It must
	 * be large enough to exclude the binders.
	 */
	CBS message;
	...
	/*
	 * Hash a ClientHello prefix up to the binders. For now, this
	 * assumes we only ever verify PSK binders on initial
	 * ClientHellos.
	 */
	uint8_t context[EVP_MAX_MD_SIZE];
	unsigned context_len;
	if (!EVP_Digest(CBS_data(&message), CBS_len(&message) -
	    CBS_len(binders) - 2, context, &context_len,
	    SSL_TRANSCRIPT_md(&hs->transcript), NULL)) {
		return 0;
	}
	...
	if (!tls13_psk_binder(verify_data, SSL_TRANSCRIPT_md(&hs->transcript),
	    ...
	    /*
	     * We only consider the first PSK, so compare against the
	     * first binder.
	     */
