...
static int
resolve_ecdhe_secret(SSL_HANDSHAKE *hs, int *out_need_retry,
SSL_CLIENT_HELLO *client_hello) {
	...
	*out_need_retry = 0;
	...
	if (!ssl_ext_key_share_parse_clienthello(hs, &found_key_share,
	    &dhe_secret, &dhe_secret_len, &alert, &key_share)) {
...
const SSL *ssl, const SSL_CLIENT_HELLO *client_hello) {
	...
	while (CBS_len(&cipher_suites) > 0) {
		...
		const SSL_CIPHER *candidate =
		    SSL_get_cipher_by_value(cipher_suite);
		if (candidate == NULL ||
		...
		/*
		 * TLS 1.3 removes legacy ciphers, so honor the client
		 * order, but prefer ChaCha20 if we do not have AES
		 * hardware.
		 */
		if (aes_is_fine) {
			return candidate;
...
static int
add_new_session_tickets(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
        /*
	 * TLS 1.3 recommends single-use tickets, so issue multiple
	 * tickets in case the client makes several connections before
	 * getting a renewal.
	 */
...
do_select_parameters(SSL_HANDSHAKE *hs) {
	/*
	 * At this point, most ClientHello extensions have already
	 * been processed by the common handshake logic. Resolve the
	 * remaining non-PSK parameters.
	 */
	SSL *const ssl = hs->ssl;
...
select_session(SSL_HANDSHAKE *hs, uint8_t *out_alert, SSL_SESSION **out_session,
...
static enum ssl_hs_wait_t
do_select_session(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
	SSL_CLIENT_HELLO client_hello;
	if (!ssl_client_hello_init(ssl, &client_hello, ssl->init_msg,
	    ssl->init_num)) {
...
static enum ssl_hs_wait_t
do_send_hello_retry_request(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_process_second_client_hello(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_send_server_hello(SSL_HANDSHAKE *hs) {
	SSL *const ssl = hs->ssl;
...
static enum ssl_hs_wait_t
do_send_server_certificate_verify(SSL_HANDSHAKE *hs,
int is_first_run) {
...
static enum ssl_hs_wait_t
do_process_channel_id(SSL_HANDSHAKE *hs) {
...
enum ssl_hs_wait_t
tls13_server_handshake(SSL_HANDSHAKE *hs) {
